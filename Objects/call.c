...
_PyObject_MakeTpCall(PyObject *callable, PyObject *const *args, Py_ssize_t nargs, PyObject *keywords)
{
    ...
    PyObject *argstuple = _PyTuple_FromArray_Borrowed(args, nargs);
    if (argstuple == NULL) {
        return NULL;
    }
                         ...
                         PyObject *kwargs)
{
    ...
    if (nk != 0) {
        ...
        kwtuple = PyTuple_New_Nonzeroed(2 * nk);
        if (kwtuple == NULL) {
            return NULL;
        }
                                 ...
                                 PyObject *kwnames)
{
    ...
    case METH_VARARGS | METH_KEYWORDS:
    {
        ...
        argtuple = _PyTuple_FromArray_Borrowed(args, nargs);
        if (argtuple == NULL) {
            goto exit;
        }
        ...
        if (flags & METH_KEYWORDS) {
            ...
            if (nkwargs > 0) {
                ...
                if (kwdict == NULL) {
                    _PyTuple_Decref_Borrowed(argtuple);
                    goto exit;
                }
            }
            else {
                    ...
                    PyObject *const **p_stack, PyObject **p_kwnames)
{
    ...
    kwnames = PyTuple_New_Nonzeroed(nkwargs);
    if (kwnames == NULL) {
        PyMem_Free(stack);
        return -1;
    }
