...
#ifndef NITROUS_INTERPRETER_INTERPRETER_H
#define NITROUS_INTERPRETER_INTERPRETER_H
...
public:
  ...
  virtual void callFunction(uint64_t addr, ArrayRef<GenericValue> ArgVals) = 0;
  virtual void run() = 0;                // Execute instructions until nothing left to do

  // dest=0 is the true case, 1 false:
  virtual void branchCallback(BranchInst*, int dest) = 0;
  // dest==-1 is the default dest
  virtual void switchCallback(SwitchInst*, int dest) = 0;
  ...
  virtual GenericValue callExternalFunction(Function *F,
                                    ArrayRef<GenericValue> ArgVals) = 0;
...
} // End llvm_interp namespace
} // End nitrous namespace
